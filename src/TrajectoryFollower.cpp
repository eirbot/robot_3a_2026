...
TrajectoryFollower::TrajectoryFollower()
    : nPoints(0), currentIdx(0), active(false), hasCorr(false), Ld(0.2f),
      v_nom(0.1f) // 20cm lookahead, 0.2m/s par défaut
{
...
bool TrajectoryFollower::hasCorrectedPose() const { return hasCorr; }

bool TrajectoryFollower::isFinished() const { return !active; }
...
void TrajectoryFollower::setLookahead(float Ld_m) { Ld = Ld_m; }

void TrajectoryFollower::setNominalSpeed(float v_mps) { v_nom = v_mps; }

void TrajectoryFollower::computeCommand(const Pose2D &poseOdom, float dt,
  ...
  // --- BRIDAGE VITESSE (CLIPPING) ---
  // Si la roue extérieure tourne trop vite dans un virage,
  // on diminue proportionnellement les deux vitesses et on augmente le temps de
  // l'arc. La consigne MAX est définie dans utilities.hpp : MAX_SPEED_MM_S (en
  // mm/s) qu'on ramène en m/s.
  float max_speed_m_s = MAX_SPEED_MM_S / 1000.0f; // Ex: 0.200 m/s

  // On prend le max absolu des deux vitesses demandées
  float abs_vL = fabs(vL);
  float abs_vR = fabs(vR);
  float max_requested = (abs_vL > abs_vR) ? abs_vL : abs_vR;

  if (max_requested > max_speed_m_s) {
    // Le facteur de réduction
    float scale = max_speed_m_s / max_requested;

    vL *= scale;
    vR *= scale;

    // Comme on va moins vite, le segment prendra plus de temps !
    temps_arc /= scale;

    Serial.print("  [CLIPPED] scale: ");
    Serial.print(scale);
  }
